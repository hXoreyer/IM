...
QNChatMessage::QNChatMessage(QWidget *parent) : QWidget(parent)
{
    ...
    m_loadingMovie->setFileName(":/Chat/image/loading4.gif");
...
void QNChatMessage::setTextSuccess(bool isSus)
{
    if(isSus)
    {
        m_loading->hide();
        ...
        m_loadingMovie->stop();
        QPixmap errPix(":/Chat/image/error.svg");
        m_loading->setPixmap(errPix.scaled(16,16));
        m_isSending = false;
    }
}

void QNChatMessage::setText(const QPixmap& headimg,const QString& text, const QString& time, const QSize& allSize, User_Type userType)
{
    m_msg = text;
    m_userType = userType;
    m_time = time;
...
void QNChatMessage::paintEvent(QPaintEvent *event)
{
    ...
    if(m_userType == User_Type::User_Other) { // 用户
        //头像
        painter.drawRoundRect(m_iconLeftRect,m_iconLeftRect.width(),m_iconLeftRect.height());
        painter.drawPixmap(m_iconLeftRect, m_headimg);
        ...
        painter.drawPixmap(m_iconRightRect, m_headimg);
